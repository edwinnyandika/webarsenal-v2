 ...
 * ║ Module: populate-missing-modules.js                                                          ║
 * ╚════════════════════════════════════════════════════════════════════════════╝
 */
...
const template = (id, category, slug, name) => `#!/usr/bin/env node
'use strict';
...
function main() {
  ...
  for (const m of modules) {
    ...
    if (!fs.existsSync(filePath)) {
      ...
      const content = template(m.id, m.category, m.slug, m.name);
      fs.writeFileSync(filePath, content, 'utf8');
      console.log(`[NEW] ${m.id} -> ${m.filePath}`);
      created++;
    } else {
      skipped++;
    }
  ...
  console.log(`\n[*] FINISHED. Created ${created} new module files. Skipped ${skipped} existing/non-shared.`);
}
